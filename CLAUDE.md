...
- **`referenciales.cbr` column was dropped (2026-05-19)**: Do not `SELECT cbr` directly — the column no longer exists in Neon. Access CBR name via `LEFT JOIN conservadores c ON r."conservadorId" = c.id` then `c.nombre AS cbr`. The `Contribution.cbr` field in Supabase (Prisma) is unaffected — it's a separate user-input table.
